...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /**
     * 只有 ctl<0 为运行状态
     * wc: work count 工作线程数量
     */
    private final AtomicInteger ctl = new AtomicInteger(ctlOf(RUNNING, 0));

    /**
     * Integer.SIZE 为 32为 ，减去的3位用来保存下面线程池的运行状态的值
     */
    ...
    /**
     * 整型变量的高3位：111   （第一位1为符号位，代表负数）
     * 接收新任务和进程队列任务
     * RUNNING -> SHUTDOWN  : 调用shutdown()
     */
    private static final int RUNNING = -1 << COUNT_BITS;
    /**
     * 整型变量的高3位：000   （第一位1为符号位，代表正数）
     * 不接收新任务，但接收进程队列任务(为执行完成的任务)
     * RUNNING or SHUTDOWN -> STOP : shutdownNow()
     * SHUTDOWN -> TIDYING : 队列和线程池都是空的
     */
    private static final int SHUTDOWN = 0 << COUNT_BITS;
    /**
     * 整型变量的高3位：001   （第一位1为符号位，代表正数）
     * 不接收新任务也不接收进程队列任务，并且打断正在进行中的任务
     * STOP -> TIDYING : 线程池为空
     */
    private static final int STOP = 1 << COUNT_BITS;
    /**
     * 整型变量的高3位：010   （第一位1为符号位，代表正数）
     * 所有任务终止，待处理任务数量为0，线程转换为TIDYING，将会执行terminated钩子函数
     * TIDYING -> TERMINATED : 钩子函数terminated()执行完成
     */
    private static final int TIDYING = 2 << COUNT_BITS;
    /**
     * 整型变量的高3位：011   （第一位1为符号位，代表正数）
     * terminated()执行完成
     */
     ...
     * 保存worker
     *
     * @see {{@link #addWorker(Runnable, boolean)}}
     */
    ...
    private void advanceRunState(int targetState) {
        for (; ; ) {
            ...
            //      当前的状态（c）至少是目标状态（targetState），若是 则break
            // ctl.compareAndSet(c, ctlOf(targetState, workerCountOf(c)))：
            //      修改当前状态，若成功则break，否则继续执行循环，直到成功
     ...
     */
    /**
     * 线程池被shutdown等时所有线程都会执行此方法
     */
    final void tryTerminate() {
        for (; ; ) {
            int c = ctl.get();
            /**
             * isRunning(c)： 判断当前线程池是不是running状态
             * runStateAtLeast(c, TIDYING) ： 当前线程池运行状态是否正在执行TIDYING钩子函数
             * runStateOf(c) ： 当前线程池状态
             */
            if (isRunning(c) ||
            ...
            /**
             * 此时队列已经为空
             * workerCountOf(c) != 0 ： 仍有线程正在活动
             * workerCountOf(c)==0 表示当前线程就是最后一个线程
             */
            if (workerCountOf(c) != 0) { // Eligible to terminate
                /**
                 * 只中断一个空闲线程（因为队列已经为空，如果中断所有的，就没有线程会执行下面的代码，即下面的代码只有一个线程时才执行）
                 */
            ...
            try {
                /**
                 * 把最后一个线程状态变为tidying，然后执行terminated()方法，执行完后再把状态变为TERMINATED
                 */
                if (ctl.compareAndSet(c, ctlOf(TIDYING, 0))) {
                    try {
                        ...
                        /**
                         * 唤醒所有等待TERMINATED的线程（等待的就是变为TERMINATED的所有线程）
                         */
    ...
    private List<Runnable> drainQueue() {
        ...
        /**
         * q.drainTo(taskList); 和 下面的 代码操作是一样的，
         * 只不过队列为延迟队列或其他情况下（如继承阻塞队列并未重写此方法时）q.drainTo(taskList)可能会失败，
         * 所以需要再手动删除并添加到列表中
         */
        q.drainTo(taskList);
        /**
         * 如果任务队列不为空，则将任务移除并添加到列表里面
         */
        if (!q.isEmpty()) {
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (; ; ) {
            ...
            int rs = runStateOf(c);
            /**
             * 如果 线程池已经结束了(意味着线程调了这几个方法：SHUTDOWN、STOP、TIDYING)
             * 且 !(线程池为SHUTDOWN 且 新提交的任务为null 且 队列不为空)
             *      SHUTDOWN： 不再接收外部的任务，但进程队列的任务会被执行完成
             *      STOP: 不接收新任务也不接收进程队列任务
             *      TIDYING: 所有任务终止，待处理任务数量为0
             *
             * 其中 && !(rs == SHUTDOWN && firstTask == null && !workQueue.isEmpty())) ：
             *      代表线程队列一定不为空
             */
        ...
        try {
            ...
            if (t != null) {
                final ReentrantLock mainLock = this.mainLock;
                /**
                 * 当多线程进来时用来保证 下面 workers （HashSet）的线程安全
                 */
                mainLock.lock();
                try {
    ...
    private Runnable getTask() {
        ...
        for (; ; ) {
            ...
            // Check if queue empty only if necessary.
            /**
             * 线程状态为shutdown等一系列退出状态时，且线程状态至少是stop或者任务队列为空时，对工作线程进行减一，并返回null
             */
            ...
            try {
                /**
                 * workQueue.take() 为阻塞方法，且会抛出中断异常（当执行shutdown等一系列关闭操作时会抛出中断异常，进入catch中）
                 */
                Runnable r = timed ?
    ...
    final void runWorker(Worker w) {
        ...
        w.firstTask = null;
        /**
         * 线程刚创建时候状态为-1 代表不能被中断，这时候线程启动则可以被中断
         * {@link Worker} 构造方法
         */
        ...
        try {
            ...
            while (task != null || (task = getTask()) != null) {
                ...
                w.lock();
                /**
                 * runStateAtLeast(ctl.get(), STOP ： 若当前线程池是否已经已经ShutDownNow了 返回 true
                 *  Thread.interrupted() ： 当前线程是否已被中断
                 *  wt.isInterrupted() ： 判断是否已经被中断了，并清除中断标记位
                 */
                if (
                        (
     ...
     * @param unit 时间单位
     * @param workQueue  只有当队列满后才会创建非核心线程数去处理任务
    ...
    public void shutdown() {
        ...
        try {
            ...
            /**
             * 中断空闲线程（因为不接收新任务，但是还执行queue任务，所以只把空闲的线程中断，剩下的线程执行queue任务）
             * @see {{@link #runWorker(Worker)}}   - >  (task = getTask()) != null ： 如果有空闲线程会被阻塞到这
             * 当中断空闲线程后 {@link #getTask()}  - > workQueue.take() : 会从这里的阻塞退出
             */
    ...
    public List<Runnable> shutdownNow() {
        ...
        try {
            ...
            /**
             * 中断所有线程（既然不接收新任务，也不执行queue队列任务，所以所有线程都要被中断）
             */
            interruptWorkers();
            /**
             * 因为不接收新任务，也不执行queue队列任务，所以把没有被执行的任务从队列里取出来，并返回
             */
            tasks = drainQueue();
        } finally {
            mainLock.unlock();
    ...
    public boolean awaitTermination(long timeout, TimeUnit unit)
        ...
        try {
            for (; ; ) {
                /**
                 * 查看线程池状态是否为TERMINATED，等待的时间是否小于0
                 */
